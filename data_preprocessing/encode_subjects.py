...
from data_preprocessing.config import MAX_SUBJECT_FEATURES, MIN_SUBJECT_FREQ
...
def build_subject_vocabulary(
    ...
    vocab = [
        subj for subj, count in counter.most_common(max_features) if count >= min_freq
    ]

    print(
        f"Subject vocabulary: {len(counter)} total -> {len(vocab)} after filtering (min_freq={min_freq})"
    )
    return vocab
...
def encode_subjects_tfidf(df: pd.DataFrame, vocab: List[str]) -> sparse.csr_matrix:
    """
